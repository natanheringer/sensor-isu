...
ISU (Integrated Sensor Unit) da TRONIK: hardware que fica no coletor, estima enchimento com sensores ToF, usa o acelerômetro para acordar em impacto e envia dados por celular (NB-IoT/LTE-M).
...
**`isu_tronik_v2.ino`** — firmware para LilyGO T-SIM7000G (ESP32 + SIM7000G), três VL53L0X, MPU6050, deep sleep entre leituras e POST HTTP. Bibliotecas e pinos estão comentados no próprio arquivo; APN, servidor e IDs ficam no bloco de configuração no topo.
...
5. *Tools → Board* → **ESP32 Dev Module** (ou o perfil da sua LilyGO, se tiver). *Tools → Port* → a COM que aparece com a placa no USB.
6. Em *Sketch → Include Library → Manage Libraries*, instale as dependências citadas no cabeçalho do `.ino` (TinyGSM, ArduinoJson, Adafruit VL53L0X, MPU6050, Unified Sensor).
7. Ajuste APN, host e IDs no topo do código, depois *Sketch → Upload*.
